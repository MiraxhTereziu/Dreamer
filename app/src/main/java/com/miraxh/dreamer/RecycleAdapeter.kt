...
class RecycleAdapeter(val context: Context, val days: List<Day>) :
    RecyclerView.Adapter<RecycleAdapeter.ViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder){
            ...
            if(date.active.not()){
                ...
                day?.setTextColor(ContextCompat.getColor(context, R.color.colorIcon))
                week?.setTextColor(ContextCompat.getColor(context, R.color.colorIcon))

            }else{
                active?.visibility = View.VISIBLE
                day?.setTextColor(ContextCompat.getColor(context, R.color.colorPrimary))
                week?.setTextColor(ContextCompat.getColor(context, R.color.colorPrimary))
            }
        }
    }
}